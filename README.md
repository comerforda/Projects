...
* This is a fire simulation using Navier-Stokes continuous fluid dynamics based on a paper by Jos Stam - "Real-Time Fluid Dynamics for Games". In this paper, Stam details how to use the Navier-Stokes fluid equations to create a smoke simulation. After successfully replicating this technique, I extended it to making a fire-simulation which added additional complexities such as bouyancy forces to cause the flames to rise, dampening to simulate particles dissipating, and attempting to extend it to a 3D environment requiring higher resolution.
