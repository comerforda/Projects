...
class JsonHelper {
 public:
  ...
  static const picojson::value& GetValue(const picojson::object& obj,
                                         std::string key) {
    return obj.find(key)->second;
  }

  static const picojson::object& GetObject(const picojson::object& obj,
                                           std::string key) {
    return GetValue(obj, key).get<picojson::object>();
  }

  static const picojson::array& GetArray(const picojson::object& obj,
                                         std::string key) {
  ...
  static void PrintKeyValues(const picojson::object& obj,
                             std::string prefix = "  ") {
    std::cout << "PrintKeyValues ---------" << std::endl;
    for (picojson::object::const_iterator it = obj.begin(); it != obj.end();
         it++) {
  ...
  static void PrintEntityDetails(const picojson::object& val) {
    ...
    std::cout << "  Contains type: " << JsonHelper::ContainsKey(val, "type")
              << std::endl;
    std::cout << "  Contains otherKey: "
              << JsonHelper::ContainsKey(val, "otherKey") << std::endl;
    std::cout << "  Contains position: "
              << JsonHelper::ContainsKey(val, "position") << std::endl;
